  ...
  template: `
    <div style="max-width:600px;margin:0 auto;padding:16px 12px 80px;">
      ...
      <div *ngIf="activePass; else choosePlan">
        ...
        <button (click)="usePassRide()" [disabled]="ridesRemaining === 0" style="width:100%;background:#1e88e5;color:#fff;border:none;border-radius:14px;padding:16px;font-size:1rem;font-weight:700;cursor:pointer;">
          ⚡ Use Pass Ride
        </button>
      </div>
      ...
      <ng-template #choosePlan>
        ...
        <div style="display:flex;flex-direction:column;gap:16px;">
          <div *ngFor="let plan of plans" [style.border]="plan.bestValue ? '2px solid #1e88e5' : '1px solid #e0e0e0'"
            ...
            <button (click)="buyPlan(plan)" style="width:100%;background:#1e88e5;color:#fff;border:none;border-radius:10px;padding:12px;font-size:0.9rem;font-weight:700;cursor:pointer;">Buy Now</button>
          </div>
        </div>
      </ng-template>
