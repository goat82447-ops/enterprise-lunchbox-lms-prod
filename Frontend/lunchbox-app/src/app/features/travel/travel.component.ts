...
type TravelMode = 'normal' | 'women' | 'teen' | 'later' | 'others' | 'pickup' | 'parcel' | 'grocery' | 'documents' | 'medicine';
...
const MODE_CONFIG: Record<TravelMode, { label: string; badge: string; badgeColor: string; badgeBg: string; icon: string; hint: string }> = {
  ...
  pickup:    { label: 'Pickup Service',         badge: '🛍️ Pickup',             badgeColor: '#e65100', badgeBg: '#fff3e0', icon: '🛍️', hint: 'Captain picks up items from a shop and delivers to you.' },
  parcel:    { label: 'Send a Parcel',          badge: '📦 Parcel',              badgeColor: '#00897b', badgeBg: '#e0f2f1', icon: '📦', hint: 'Send a package from pickup to drop location.' },
  grocery:   { label: 'Grocery Delivery',       badge: '🛒 Grocery',             badgeColor: '#2e7d32', badgeBg: '#e8f5e9', icon: '🛒', hint: 'Captain shops and delivers your grocery list.' },
  documents: { label: 'Document Delivery',      badge: '📄 Documents',           badgeColor: '#1565c0', badgeBg: '#e3f2fd', icon: '📄', hint: 'Secure and confidential document delivery.' },
  medicine:  { label: 'Medicine Delivery',      badge: '💊 Medicine',            badgeColor: '#6a1b9a', badgeBg: '#f3e5f5', icon: '💊', hint: 'Pharmacy pickup with prescription verification.' },
};
      ...
      <div class="tx-vehicles">
        ...
        >
          <div class="tx-veh-left">
            <!-- Delivery service icon replaces plain emoji -->
            <div class="tx-veh-emoji" [style.background]="v.color + '18'">
              <!-- Parcel / Pickup: delivery person carrying box -->
              <ng-container *ngIf="isDeliveryMode">
                <svg *ngIf="mode==='parcel' || mode==='pickup' || mode==='grocery'" width="34" height="34" viewBox="0 0 64 64" fill="none">
                  <!-- body -->
                  <circle cx="32" cy="14" r="8" fill="#ffcc80"/>
                  <rect x="22" y="24" width="20" height="18" rx="4" fill="#1565c0"/>
                  <!-- arms -->
                  <rect x="10" y="26" width="12" height="5" rx="2.5" fill="#1565c0"/>
                  <rect x="42" y="26" width="12" height="5" rx="2.5" fill="#1565c0"/>
                  <!-- box held -->
                  <rect x="8" y="28" width="14" height="12" rx="2" fill="#f59e0b"/>
                  <rect x="10" y="30" width="10" height="2" rx="1" fill="#fbbf24"/>
                  <!-- legs -->
                  <rect x="24" y="42" width="7" height="14" rx="3" fill="#1565c0"/>
                  <rect x="33" y="42" width="7" height="14" rx="3" fill="#1565c0"/>
                  <!-- vehicle emoji small -->
                  <text x="38" y="60" font-size="14">{{ v.emoji }}</text>
                </svg>

                <!-- Medicine: medical bag + capsule -->
                <svg *ngIf="mode==='medicine'" width="34" height="34" viewBox="0 0 64 64" fill="none">
                  <rect x="12" y="20" width="40" height="32" rx="6" fill="#e3f2fd" stroke="#1565c0" stroke-width="2"/>
                  <rect x="26" y="12" width="12" height="12" rx="4" fill="#fff" stroke="#1565c0" stroke-width="2"/>
                  <rect x="29" y="26" width="6" height="20" rx="3" fill="#e53935"/>
                  <rect x="20" y="33" width="24" height="6" rx="3" fill="#e53935"/>
                  <text x="38" y="62" font-size="12">{{ v.emoji }}</text>
                </svg>

                <!-- Documents: folder with paper -->
                <svg *ngIf="mode==='documents'" width="34" height="34" viewBox="0 0 64 64" fill="none">
                  <rect x="8" y="22" width="48" height="34" rx="5" fill="#fff9c4" stroke="#f59e0b" stroke-width="2"/>
                  <path d="M8 30 Q8 22 16 22 H26 L30 26 H52 Q56 26 56 30" fill="#f59e0b"/>
                  <rect x="18" y="34" width="28" height="3" rx="1.5" fill="#b45309"/>
                  <rect x="18" y="40" width="20" height="3" rx="1.5" fill="#b45309"/>
                  <rect x="18" y="46" width="24" height="3" rx="1.5" fill="#b45309"/>
                  <text x="40" y="62" font-size="12">{{ v.emoji }}</text>
                </svg>
              </ng-container>

              <!-- Normal ride modes: just show emoji -->
              <ng-container *ngIf="!isDeliveryMode">{{ v.emoji }}</ng-container>
            </div>
            <div class="tx-veh-info">
              <div class="tx-veh-name">
                <!-- Service-specific label prefix -->
                <span *ngIf="isDeliveryMode">{{ deliveryVehicleLabel(v) }}</span>
                <span *ngIf="!isDeliveryMode">{{ v.label }}</span>
              ...
              <div class="tx-veh-meta">{{ v.etaMin }} min • {{ v.capacity }} {{ isDeliveryMode ? 'kg max' : 'seat(s)' }} • {{ v.description }}</div>
            </div>
          </div>
          <div class="tx-veh-right">
            <div class="tx-veh-fare">₹{{ calculateFare(v) }}</div>
            <div class="tx-veh-drop">{{ isDeliveryMode ? 'Delivers' : 'Drop' }} {{ getDropTime(v.etaMin) }}</div>
          </div>
        </div>
      </div>
...
export class TravelComponent implements OnInit, OnDestroy {
  ...
  get isDeliveryMode(): boolean {
    return ['parcel', 'pickup', 'grocery', 'medicine', 'documents'].includes(this.mode);
  }

  deliveryVehicleLabel(v: { type: string; label: string }): string {
    ...
    const prefix = modePrefix[this.mode] || '';
    return prefix ? `${prefix} ${v.label}` : v.label;
  }
  ...
  readonly vehicleOptions: VehicleOption[] = [
    { type: 'scotty', emoji: '🛵',  label: 'Scotty',       description: 'Cheapest · upto 5kg',    farePerKm: 6,  baseKm: 15, etaMin: 4, capacity: 1, tag: 'CHEAPEST', color: '#00897b' },
    { type: 'bike',   emoji: '🏍️', label: 'Bike',         description: 'Quick · upto 10kg',       farePerKm: 8,  baseKm: 20, etaMin: 3, capacity: 1, color: '#ff6f00' },
    { type: 'auto',   emoji: '🛺',  label: 'Auto',         description: 'Affordable · upto 20kg',  farePerKm: 12, baseKm: 25, etaMin: 1, capacity: 3, tag: 'FASTEST', color: '#2e7d32' },
    { type: 'cab',    emoji: '🚗',  label: 'Cab Economy',  description: 'Comfortable · upto 40kg', farePerKm: 18, baseKm: 30, etaMin: 5, capacity: 4, color: '#1565c0' },
    { type: 'prime',  emoji: '🚙',  label: 'Cab Prime',    description: 'Premium · upto 60kg',     farePerKm: 24, baseKm: 40, etaMin: 6, capacity: 4, color: '#6a1b9a' },
  ];
