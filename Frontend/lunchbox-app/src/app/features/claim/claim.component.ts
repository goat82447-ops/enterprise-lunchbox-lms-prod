    ...
    <div style="max-width:600px;margin:0 auto;padding:16px 12px 80px;">
      ...
      <div style="background:#fff;border-radius:16px;padding:20px;margin-bottom:20px;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
        ...
        <div *ngIf="submitSuccess" style="background:#ecfdf5;border:1px solid #6ee7b7;border-radius:10px;padding:12px 16px;color:#065f46;font-size:0.9rem;margin-bottom:16px;">
          ✅ Your claim has been submitted! We'll get back to you shortly.
        </div>

        <div style="margin-bottom:14px;">
          <label style="font-size:0.85rem;font-weight:600;color:#444;display:block;margin-bottom:8px;">Claim Type</label>
        ...
        <div style="margin-bottom:14px;">
          ...
          <input [(ngModel)]="newClaim.bookingId" placeholder="e.g. RX20240001" style="width:100%;border:1px solid #e5e7eb;border-radius:10px;padding:10px 14px;font-size:0.9rem;outline:none;box-sizing:border-box;">
        </div>

        <div style="margin-bottom:14px;">
          <label style="font-size:0.85rem;font-weight:600;color:#444;display:block;margin-bottom:6px;">Description</label>
          <textarea [(ngModel)]="newClaim.description" rows="3" placeholder="Describe your issue..." style="width:100%;border:1px solid #e5e7eb;border-radius:10px;padding:10px 14px;font-size:0.9rem;outline:none;resize:none;box-sizing:border-box;"></textarea>
        </div>

        <div style="margin-bottom:20px;">
          <label style="font-size:0.85rem;font-weight:600;color:#444;display:block;margin-bottom:8px;">Contact Preference</label>
        ...
        <button (click)="submitClaim()" [disabled]="!newClaim.claimType || !newClaim.description" style="width:100%;background:#f97316;color:#fff;border:none;border-radius:12px;padding:14px;font-size:1rem;font-weight:700;cursor:pointer;" [style.opacity]="!newClaim.claimType || !newClaim.description ? '0.6' : '1'">
          Submit Claim
        </button>
      </div>
      ...
      <div *ngIf="claimsHistory.length > 0">
        ...
        <div *ngFor="let claim of claimsHistory" style="background:#fff;border-radius:14px;padding:16px;margin-bottom:12px;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
            ...
            <span [style.background]="statusColor(claim.status)" style="color:#fff;border-radius:20px;padding:4px 12px;font-size:0.72rem;font-weight:700;">{{ claim.status }}</span>
          </div>
