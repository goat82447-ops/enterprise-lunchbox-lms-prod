...
export class NotificationService {
  ...
  push(message: string, level: AppNotification['level'] = 'info'): void {
    const notification: AppNotification = {
      ...
      message, level, createdAt: new Date().toISOString(), read: false
    };
    this.notificationCenterSubject.next([notification, ...this.notificationCenterSubject.value].slice(0, 120));
    this.notificationsSubject.next([notification, ...this.notificationsSubject.value].slice(0, 6));
    ...
    setTimeout(() => { this.dismiss(notification.id); }, this.autoDismissMs);
  }
    ...
    const run = () => {
      if (type === 'alert') {
        ...
        [{ f:880,s:0,d:0.12 },{ f:1320,s:0.14,d:0.2 }].forEach(b => {
          const o = ctx.createOscillator(), g = ctx.createGain();
          o.type = 'sine'; o.frequency.value = b.f;
          g.gain.setValueAtTime(0.0001, ctx.currentTime+b.s);
  ...
  private pushBrowserNotification(notification: AppNotification): void {
    ...
    new Notification(this.resolveTitle(notification.level), { body: notification.message, tag: `delivery-${notification.id}` });
  }
