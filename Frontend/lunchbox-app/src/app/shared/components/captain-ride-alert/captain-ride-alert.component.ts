...
@Component({
  ...
  `,
  styles: [`
    .cp-alert-overlay { position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.75);display:flex;align-items:flex-end;justify-content:center;animation:caFadeIn .2s ease; }
    @keyframes caFadeIn { from{opacity:0}to{opacity:1} }
    .cp-alert-sheet { width:100%;max-width:480px;background:#fff;border-radius:28px 28px 0 0;padding:24px 20px 36px;animation:caSlideUp .28s ease;text-align:center; }
    @keyframes caSlideUp { from{transform:translateY(50px);opacity:.4}to{transform:translateY(0);opacity:1} }
    ...
    .cp-alert-pulse-ring { position:absolute;inset:0;border-radius:50%;border:3px solid #e53935;opacity:.5;animation:caRingGrow 1.4s ease-out infinite; }
    .cp-ring2 { animation-delay:.7s; }
    @keyframes caRingGrow { 0%{transform:scale(.8);opacity:.6}100%{transform:scale(1.7);opacity:0} }
    .cp-alert-icon { font-size:40px;z-index:1; }
    .cp-alert-badge { display:inline-block;background:#e53935;color:#fff;font-size:11px;font-weight:800;letter-spacing:.8px;padding:4px 12px;border-radius:20px;margin-bottom:8px;animation:caBadgePulse 1s ease-in-out infinite; }
    @keyframes caBadgePulse { 0%,100%{box-shadow:0 0 0 0 rgba(229,57,53,.4)}50%{box-shadow:0 0 0 8px rgba(229,57,53,0)} }
    ...
    @keyframes caAcceptPulse { 0%,100%{box-shadow:0 4px 14px rgba(76,175,80,.4)}50%{box-shadow:0 6px 20px rgba(76,175,80,.7)} }
  `]
})
